...
async function geo(a) { try { const r = await fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(a)}&limit=1`, { headers: { "User-Agent": "MasterpieceMileageApp/1.0" } }); if (!r.ok) { console.error("Geo HTTP error", r.status); return null; } const d = await r.json(); console.log("Geo result for", a, d); return d?.length ? { lat: +d[0].lat, lng: +d[0].lon } : null; } catch (e) { console.error("Geo failed", a, e); return null; } }
