...
async function geo(a) { const r = await fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(a)}&limit=1`, { headers: { "User-Agent": "MasterpieceMileageApp/1.0" } }); const d = await r.json(); return d?.length ? { lat: +d[0].lat, lng: +d[0].lon } : null; }
async function getRoute(f, t) { const a = await geo(f), b = await geo(t); if (!a || !b) return null; const r = await fetch(`https://router.project-osrm.org/route/v1/driving/${a.lng},${a.lat};${b.lng},${b.lat}?overview=false`); const d = await r.json(); return d.code === "Ok" && d.routes?.length ? Math.round(d.routes[0].distance * 0.000621371 * 10) / 10 : null; }
