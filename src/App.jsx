...
async function geo(a) { try { const r = await fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(a)}&limit=1`, { headers: { "User-Agent": "MasterpieceMileageApp/1.0" } }); if (!r.ok) return null; const d = await r.json(); if (d?.length) return { lat: +d[0].lat, lng: +d[0].lon }; const short = a.replace(/\d{5}(-\d{4})?,?\s*/g, "").replace(/,?\s*USA$/i, "").trim(); if (short !== a) { const r2 = await fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(short)}&limit=1`, { headers: { "User-Agent": "MasterpieceMileageApp/1.0" } }); const d2 = await r2.json(); if (d2?.length) return { lat: +d2[0].lat, lng: +d2[0].lon }; } return null; } catch (e) { console.error("Geo failed", a, e); return null; } }
async function getRoute(f, t) { try { const a = await geo(f), b = await geo(t); console.log("Geo A:", a, "Geo B:", b); if (!a || !b) return null; const r = await fetch(`https://router.project-osrm.org/route/v1/driving/${a.lng},${a.lat};${b.lng},${b.lat}?overview=false`); const d = await r.json(); console.log("OSRM result:", d); return d.code === "Ok" && d.routes?.length ? Math.round(d.routes[0].distance * 0.000621371 * 10) / 10 : null; } catch (e) { console.error("Route failed", e); return null; } }
