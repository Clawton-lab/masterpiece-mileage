...
let _gReady = false;
function loadGMaps() { if (_gReady || document.getElementById("gmaps-js")) return Promise.resolve(); return new Promise((res) => { const s = document.createElement("script"); s.id = "gmaps-js"; s.src = `https://maps.googleapis.com/maps/api/js?key=${GKEY}&libraries=routes`; s.onload = () => { _gReady = true; res(); }; document.head.appendChild(s); }); }
async function getRoute(f, t) { try { await loadGMaps(); return new Promise((res) => { const svc = new window.google.maps.DirectionsService(); svc.route({ origin: f, destination: t, travelMode: "DRIVING" }, (result, status) => { if (status === "OK" && result.routes?.length) { const meters = result.routes[0].legs[0].distance.value; res(Math.round(meters * 0.000621371 * 10) / 10); } else { console.error("Directions failed:", status); res(null); } }); }); } catch (e) { console.error("Route error:", e); return null; } }
