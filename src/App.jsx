...
async function geo(a) { const r = await fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(a)}&limit=1`); const d = await r.json(); return d?.length ? { lat: +d[0].lat, lng: +d[0].lon } : null; }
async function route(f, t) { const a = await geo(f), b = await geo(t); if (!a || !b) return null; const r = await fetch(`https://router.project-osrm.org/route/v1/driving/${a.lng},${a.lat};${b.lng},${b.lat}?overview=false`); const d = await r.json(); return d.code === "Ok" && d.routes?.length ? Math.round(d.routes[0].distance * 0.000621371 * 10) / 10 : null; }
...
function Nav({ tab, set, isAdmin }) {
  ...
  return <nav style={{ position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 900, background: "#fff", borderTop: `2px solid ${P.tn}`, display: "flex", justifyContent: "space-around", padding: "8px 0 env(safe-area-inset-bottom,8px)" }}>
    {ts.map(t => <button key={t.k} onClick={() => set(t.k)} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 2, padding: "4px 8px", background: "none", border: "none", cursor: "pointer", color: tab === t.k ? P.r : P.l, borderTop: tab === t.k ? `2px solid ${P.r}` : "2px solid transparent", marginTop: -2, minWidth: 0 }}><span style={{ fontSize: 10, fontWeight: 700, fontFamily: F.m }}>{t.l}</span></button>)}
  </nav>;
}
...
export default function App() {
  ...
  const [tab, setTab] = useState("log"); const [fromId, setFI] = useState(""); const [toId, setTI] = useState(""); const [tNote, setTN] = useState(""); const [calc, setCalc] = useState(false);
  ...
  const [adminPage, setAdPg] = useState("hub"); const [adminAuth, setAdAuth] = useState(false);
  ...
  const [editUser, setEditUser] = useState(null); const [euName, setEUN] = useState(""); const [euEmail, setEUE] = useState(""); const [euPin, setEUP] = useState("");
  ...
  const signup = async () => { setAErr(""); if (!aName.trim() || !aEmail.trim() || aPin.length !== 4) { setAErr("Fill all fields. PIN = 4 digits."); return; } try { const ex = await db(`yard_users?email=eq.${encodeURIComponent(aEmail.toLowerCase().trim())}`); if (ex?.length) { setAErr("Email registered. Log in instead."); return; } const r = await db("yard_users", { method: "POST", body: JSON.stringify({ email: aEmail.toLowerCase().trim(), name: aName.trim(), pin: aPin, role: "user" }) }); if (r?.[0]) { setUser(r[0]); show(`Welcome!`); } } catch (e) { setAErr("Failed."); } };

  // Trip
  const logTrip = async () => { if (!fromId || !toId || fromId === toId) { show("Select two different projects"); return; } const fp = projs.find(p => p.id === fromId), tp = projs.find(p => p.id === toId); if (!fp?.address || !tp?.address) { show("Both need addresses"); return; } setCalc(true); try { const mi = await route(fp.address, tp.address); if (!mi) { show("Couldn't calculate route"); setCalc(false); return; } const re = Math.round(mi * sett.irs_rate * 100) / 100; await db("trips", { method: "POST", body: JSON.stringify({ user_id: user.id, user_name: user.name, from_project_id: fp.id, from_project_name: fp.name, from_address: fp.address, to_project_id: tp.id, to_project_name: tp.name, to_address: tp.address, miles: mi, reimbursement: re, irs_rate: sett.irs_rate, note: tNote, trip_date: td() }) }); await load(); setFI(toId); setTI(""); setTN(""); show(`${mi} mi — $${re.toFixed(2)}`); } catch (e) { show("Error"); } setCalc(false); };

  // Admin actions
  const saveProj = async () => { if (!nPN.trim() || !nPA.trim()) { show("Need name + address"); return; } try { await db("projects", { method: "POST", body: JSON.stringify({ name: nPN.trim(), address: nPA.trim() }) }); await load(); setPM(false); setNPN(""); setNPA(""); show("Added"); } catch (e) { show("Error"); } };
  ...
  const saveEditUser = async () => { if (!euName.trim() || !euEmail.trim() || euPin.length !== 4) { show("Fill all fields"); return; } try { await db(`yard_users?id=eq.${editUser}`, { method: "PATCH", body: JSON.stringify({ name: euName.trim(), email: euEmail.toLowerCase().trim(), pin: euPin }) }); await load(); setEditUser(null); show("Employee updated"); } catch (e) { show("Error saving"); } };
  ...
  const mo = getMo(td());
  const tdT = my.filter(t => t.trip_date === td());
  const ppT = my.filter(t => t.trip_date >= pp.s && t.trip_date <= pp.e);
  const moT = my.filter(t => t.trip_date >= mo.s && t.trip_date <= mo.e);
  const ytdT = my.filter(t => t.trip_date >= `${yr()}-01-01`);
  ...
  if (!user) return <div style={{ minHeight: "100vh", background: P.bg, display: "flex", alignItems: "center", justifyContent: "center", padding: 24, fontFamily: F.b }}><style>{css}</style>
    <div style={{ width: "100%", maxWidth: 380, animation: "fadeIn .5s ease" }}>
      ...
      <div style={{ textAlign: "center", marginBottom: 32 }}><div style={{ display: "inline-flex" }}><Logo /></div><p style={{ fontSize: 13, color: P.m, marginTop: 8 }}>Outdoor Living — Mileage Tracker</p></div>
      <div style={{ display: "flex", background: P.bdL, borderRadius: 10, padding: 3, marginBottom: 24 }}>
        {["login", "signup"].map(m => <button key={m} onClick={() => { setMode(m); setAErr(""); }} style={{ flex: 1, padding: 10, borderRadius: 8, border: "none", background: mode === m ? "#fff" : "transparent", color: mode === m ? P.tx : P.l, fontSize: 14, fontWeight: 600, cursor: "pointer", fontFamily: F.b, boxShadow: mode === m ? "0 1px 3px rgba(0,0,0,.1)" : "none" }}>{m === "login" ? "Log In" : "Sign Up"}</button>)}
      </div>
  ...
  return <div style={{ minHeight: "100vh", background: P.bg, fontFamily: F.b, paddingBottom: 80 }}><style>{css}</style>
    ...
    <div style={{ padding: "16px 16px 0" }}>
      ...
      {tab === "log" && <div style={{ animation: "fadeIn .3s" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 6, marginBottom: 20 }}>
          {[{ l: "Today", v: sum(tdT), b: P.tn }, { l: "Pay Per.", v: sum(ppT), b: P.r }, { l: "Month", v: sum(moT), b: P.am }, { l: "YTD", v: sum(ytdT), b: P.bk }].map(s =>
            <div key={s.l} style={{ background: "#fff", borderRadius: 10, padding: "10px", border: `1px solid ${P.bd}`, borderLeft: `3px solid ${s.b}` }}>
              <div style={{ fontSize: 9, fontFamily: F.m, color: P.l, textTransform: "uppercase", letterSpacing: .8 }}>{s.l}</div>
              <div style={{ fontSize: 17, fontWeight: 700, fontFamily: F.h }}>{s.v.toFixed(1)}<span style={{ fontSize: 9, color: P.m }}> mi</span></div>
            </div>)}
        </div>
      ...
      {tab === "trips" && <div style={{ animation: "fadeIn .3s" }}>
        ...
        <div style={{ background: "#fff", borderRadius: 16, padding: 20, border: `1px solid ${P.bd}`, borderTop: `3px solid ${P.r}`, marginBottom: 16 }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
            {[{ l: "Pay Period", v: sum(ppT), r: sett.irs_rate }, { l: "Month", v: sum(moT), r: sett.irs_rate }, { l: "YTD", v: sum(ytdT), r: sett.irs_rate }].map(s =>
              <div key={s.l}><div style={{ fontSize: 10, fontFamily: F.m, color: P.l, textTransform: "uppercase" }}>{s.l}</div><div style={{ fontSize: 24, fontWeight: 700, fontFamily: F.h }}>{s.v.toFixed(1)}</div><div style={{ fontSize: 12, color: P.g, fontFamily: F.m, fontWeight: 600 }}>${(s.v * s.r).toFixed(2)}</div></div>)}
          </div>
        </div>
        {grpDay(my).length === 0 ? <div style={{ padding: 40, textAlign: "center", color: P.l, fontFamily: F.m }}>No trips yet</div> : grpDay(my).map(d => <div key={d.date} style={{ marginBottom: 14 }}>
      ...
      {tab === "map" && <div style={{ animation: "fadeIn .3s" }}>
        ...
        {!mapLoad && mapPins.length > 0 && (() => {
          const pts = [...mapPins]; if (myLoc) pts.push(myLoc);
          const lats = pts.map(p => p.lat), lngs = pts.map(p => p.lng);
          const mnLa = Math.min(...lats), mxLa = Math.max(...lats), mnLo = Math.min(...lngs), mxLo = Math.max(...lngs);
          const pd = Math.max((mxLa - mnLa) * .15, .01), pL = Math.max((mxLo - mnLo) * .15, .01);
          const W = 600, H = 400, cLa = (mnLa + mxLa) / 2, cLo = (mnLo + mxLo) / 2, sLa = (mxLa - mnLa) + pd * 2, sLo = (mxLo - mnLo) + pL * 2;
          const tX = lng => ((lng - (cLo - sLo / 2)) / sLo) * W, tY = lat => H - ((lat - (cLa - sLa / 2)) / sLa) * H;
          return <div>
            <div style={{ borderRadius: 16, overflow: "hidden", border: `1px solid ${P.bd}`, position: "relative" }}>
              <iframe title="Map" width="100%" height="400" frameBorder="0" style={{ display: "block" }} src={`https://www.openstreetmap.org/export/embed.html?bbox=${mnLo - pL},${mnLa - pd},${mxLo + pL},${mxLa + pd}&layer=mapnik`} />
              <svg viewBox={`0 0 ${W} ${H}`} style={{ position: "absolute", top: 0, left: 0, width: "100%", height: "100%", pointerEvents: "none" }}>
                {mapPins.map(p => <g key={p.id} style={{ pointerEvents: "all", cursor: "pointer" }} onClick={() => setSP(selPin === p.id ? null : p.id)}>
                  <circle cx={tX(p.lng)} cy={tY(p.lat)} r={selPin === p.id ? 10 : 7} fill={P.r} stroke="#fff" strokeWidth="2.5" />
                  {selPin === p.id && <><rect x={tX(p.lng) - 55} y={tY(p.lat) - 30} width="110" height="20" rx="4" fill="rgba(0,0,0,.85)" /><text x={tX(p.lng)} y={tY(p.lat) - 16} textAnchor="middle" fontSize="11" fontWeight="700" fill="#fff" fontFamily={F.m}>{p.name}</text></>}
                </g>)}
                {myLoc && <><circle cx={tX(myLoc.lng)} cy={tY(myLoc.lat)} r="8" fill="rgba(37,99,235,.3)" /><circle cx={tX(myLoc.lng)} cy={tY(myLoc.lat)} r="5" fill="#2563eb" stroke="#fff" strokeWidth="2" /></>}
              </svg>
            </div>
      ...
      {tab === "projects" && <div style={{ animation: "fadeIn .3s" }}>
        ...
        {projs.filter(p => pFilter === "active" ? p.active !== false : pFilter === "archived" ? p.active === false : true).map(p => <div key={p.id} style={{ padding: "12px 16px", background: "#fff", borderRadius: 12, border: `1px solid ${P.bd}`, marginBottom: 8, borderLeft: `3px solid ${p.active !== false ? P.tn : P.l}`, opacity: p.active !== false ? 1 : .6 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
            <div><div style={{ fontWeight: 700, fontSize: 15 }}>{p.name}</div><div style={{ fontSize: 12, color: P.l, marginTop: 2 }}>{p.address || "No address"}</div></div>
      ...
      {tab === "admin" && isA && <div style={{ animation: "fadeIn .3s" }}>
        ...
        {!adminAuth && <div style={{ maxWidth: 340, margin: "40px auto", textAlign: "center" }}>
          ...
          <Fl l="PIN"><input style={{ ...iS, textAlign: "center", fontSize: 24, letterSpacing: 12, fontFamily: F.m }} maxLength={4} value={aaPin} onChange={e => setAAP(e.target.value.replace(/\D/g, "").slice(0, 4))} placeholder="----" onKeyDown={e => { if (e.key === "Enter" && aaPin.length === 4) { if (aaName.toLowerCase() === user.name.toLowerCase() && aaPin === user.pin) { setAdAuth(true); setAAE(""); } else setAAE("Invalid."); } }} /></Fl>
          {aaErr && <div style={{ color: P.r, fontSize: 13, marginBottom: 12, fontFamily: F.m }}>{aaErr}</div>}
          <Btn full onClick={() => { if (aaName.toLowerCase() === user.name.toLowerCase() && aaPin === user.pin) { setAdAuth(true); } else setAAE("Invalid credentials."); }}>Unlock Admin</Btn>
        </div>}

        {/* Hub */}
        {adminAuth && adminPage === "hub" && <div>
          <h2 style={{ fontFamily: F.h, fontSize: 20, fontWeight: 700, margin: "0 0 20px" }}>Admin</h2>
          {[{ k: "reports", l: "Reports", d: "View & send mileage reports", c: P.r }, { k: "settings", l: "Settings", d: "IRS rate, pay periods", c: P.tn }, { k: "users", l: "Employees", d: "Manage team members", c: P.bk }].map(p =>
            <button key={p.k} onClick={() => setAdPg(p.k)} style={{ display: "block", width: "100%", textAlign: "left", padding: "16px 20px", background: "#fff", borderRadius: 14, border: `1px solid ${P.bd}`, borderLeft: `4px solid ${p.c}`, marginBottom: 10, cursor: "pointer", fontFamily: F.b }}>
              <div style={{ fontWeight: 700, fontSize: 16 }}>{p.l}</div><div style={{ fontSize: 13, color: P.l, marginTop: 4 }}>{p.d}</div>
            </button>)}
        </div>}
        ...
        {adminAuth && adminPage === "reports" && <div>
          ...
          {!rEmpDet && <>
            <select value={rPer} onChange={e => setRP(e.target.value)} style={{ ...iS, marginBottom: 16 }}>
              <option value="current">Current Pay Period</option><option value="month">This Month</option><option value="ytd">Year to Date</option><option value="all">All Time</option>
            </select>
            <h3 style={{ fontFamily: F.h, fontSize: 15, fontWeight: 700, margin: "0 0 10px" }}>Employees</h3>
            {empSum().map(e => <button key={e.id} onClick={() => setRED(e.id)} style={{ display: "block", width: "100%", textAlign: "left", padding: "14px 16px", background: "#fff", borderRadius: 12, border: `1px solid ${P.bd}`, marginBottom: 8, cursor: "pointer", fontFamily: F.b }}>
              <div style={{ display: "flex", justifyContent: "space-between" }}><div><div style={{ fontWeight: 700, fontSize: 15 }}>{e.name}</div><div style={{ fontSize: 12, color: P.l, fontFamily: F.m }}>{e.ct} trips</div></div><div style={{ textAlign: "right" }}><div style={{ fontSize: 18, fontWeight: 700, fontFamily: F.h }}>{e.mi.toFixed(1)} mi</div><div style={{ fontSize: 13, color: P.g, fontFamily: F.m }}>${e.re.toFixed(2)}</div></div></div>
            </button>)}
            {empSum().length === 0 && <div style={{ padding: 30, textAlign: "center", color: P.l, fontFamily: F.m }}>No trips this period</div>}
            <Btn small color={P.bk} onClick={() => dlCSV(mkCSV(rTrips), "all-employees")} sx={{ marginTop: 12 }}>Download All (CSV)</Btn>
          </>}

          {rEmpDet && (() => {
            const et = rTrips.filter(t => t.user_id === rEmpDet); const en = et[0]?.user_name || ""; const days = grpDay(et);
            return <div>
              ...
              <div style={{ background: "#fff", borderRadius: 14, padding: 16, border: `1px solid ${P.bd}`, borderTop: `3px solid ${P.r}`, marginBottom: 16 }}>
                ...
                <div style={{ display: "flex", gap: 20 }}>
                  <div><div style={{ fontSize: 10, fontFamily: F.m, color: P.l, textTransform: "uppercase" }}>Miles</div><div style={{ fontSize: 24, fontWeight: 700, fontFamily: F.h }}>{sum(et).toFixed(1)}</div></div>
                  <div><div style={{ fontSize: 10, fontFamily: F.m, color: P.l, textTransform: "uppercase" }}>Reimb</div><div style={{ fontSize: 24, fontWeight: 700, fontFamily: F.h, color: P.g }}>${sumR(et).toFixed(2)}</div></div>
                  <div><div style={{ fontSize: 10, fontFamily: F.m, color: P.l, textTransform: "uppercase" }}>Trips</div><div style={{ fontSize: 24, fontWeight: 700, fontFamily: F.h }}>{et.length}</div></div>
                </div>
              </div>
              ...
              {days.map(d => <div key={d.date} style={{ marginBottom: 12 }}>
                ...
                {d.trips.map((t, i) => <div key={t.id} style={{ padding: "10px 14px", background: "#fff", border: `1px solid ${P.bd}`, borderTop: "none", borderRadius: i === d.trips.length - 1 ? "0 0 10px 10px" : 0 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}><div style={{ fontSize: 14, fontWeight: 600 }}>{t.from_project_name} → {t.to_project_name}</div><div style={{ textAlign: "right", flexShrink: 0 }}><span style={{ fontWeight: 700, fontFamily: F.h }}>{(+t.miles).toFixed(1)} mi</span><span style={{ fontSize: 11, color: P.g, fontFamily: F.m, marginLeft: 6 }}>${(+t.reimbursement).toFixed(2)}</span></div></div>
        ...
        {adminAuth && adminPage === "settings" && <div>
          ...
          <div style={{ background: "#fff", borderRadius: 14, border: `1px solid ${P.bd}`, padding: 16, marginBottom: 16, borderTop: `3px solid ${P.r}` }}>
          ...
          <div style={{ background: "#fff", borderRadius: 14, border: `1px solid ${P.bd}`, padding: 16, marginBottom: 16, borderTop: `3px solid ${P.tn}` }}>
            ...
            <div style={{ fontSize: 12, color: P.l, fontFamily: F.m }}>Start of any past pay period. App calculates all from here.</div>
          </div>
            ...
            return <div key={u.id} style={{ padding: "12px 16px", background: "#fff", borderRadius: 12, border: `1px solid ${P.bd}`, marginBottom: 8, borderLeft: `3px solid ${u.active ? P.g : P.l}` }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                ...
                {canE && !self && <div style={{ display: "flex", flexDirection: "column", gap: 4, alignItems: "flex-end" }}>
                  <button onClick={() => { setEditUser(u.id); setEUN(u.name); setEUE(u.email); setEUP(u.pin); }} style={{ fontSize: 11, padding: "4px 10px", borderRadius: 6, border: `1px solid ${P.bd}`, background: "#fff", color: P.m, cursor: "pointer", fontFamily: F.m }}>Edit</button>
    ...
    <Modal open={emailMod} onClose={() => setEM(false)} title="Email Report">
      ...
      <Btn full disabled={!emailTo.trim()} onClick={() => { const et = rEmpDet ? rTrips.filter(t => t.user_id === rEmpDet) : rTrips; emailRpt(et, rEmpDet ? et[0]?.user_name : null); }}>Send Report</Btn>
    </Modal>

    <Modal open={!!editUser} onClose={() => setEditUser(null)} title="Edit Employee">
      <Fl l="Name"><input style={iS} value={euName} onChange={e => setEUN(e.target.value)} /></Fl>
      <Fl l="Email"><input style={iS} type="email" value={euEmail} onChange={e => setEUE(e.target.value)} /></Fl>
      <Fl l="PIN (4 digits)"><input style={{ ...iS, textAlign: "center", fontSize: 20, letterSpacing: 10, fontFamily: F.m }} maxLength={4} value={euPin} onChange={e => setEUP(e.target.value.replace(/\D/g, "").slice(0, 4))} /></Fl>
      <Btn full disabled={!euName.trim() || !euEmail.trim() || euPin.length !== 4} onClick={saveEditUser}>Save Changes</Btn>
    </Modal>
